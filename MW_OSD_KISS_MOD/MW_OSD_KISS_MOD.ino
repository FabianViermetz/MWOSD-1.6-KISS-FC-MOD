...
//---
  ...
  #elif defined FORCESENSORS_MINIMUM
...
void loop()
{
  ...
  if((currentMillis - previous_millis_sync) >= sync_speed_cycle)  // (Executed > NTSC/PAL hz 33ms)
  {
    ...
    if(!fontMode)
      WriteGerneralRequest(MSP_ATTITUDE,0);
  }
...
// to prevent issues with high pulse RSSi consuming CPU
  ...
  if((currentMillis - previous_millis_high) >= hi_speed_cycle)  // 20 Hz or 100hz in MSP high mode
  {
    ...
    if(!fontMode){
      #ifndef GPSOSD
      WriteGerneralRequest(MSPcmdsend, 0);      
      #endif //GPSOSD
      MAX7456_DrawScreen();
...
#if defined TEMPSENSOR || defined KISS_ESC_TEMP
        if(((temperature>=TEMPERATUREMAX)&&(timer.Blink2hz))) displayTemperature();
...
/*
    if((timer.accCalibrationTimer==1)&&(configMode)) {
      WriteGerneralRequest(MSP_ACC_CALIBRATION,0);
      timer.accCalibrationTimer=0;
    }
*/    
    if((timer.magCalibrationTimer==1)&&(configMode)) {
      WriteGerneralRequest(MSP_MAG_CALIBRATION,0);
